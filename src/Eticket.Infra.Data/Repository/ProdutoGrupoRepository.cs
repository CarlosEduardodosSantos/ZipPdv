...
namespace Eticket.Infra.Data.Repository
{
    public class ProdutoGrupoRepository : RepositoryBase, IProdutoGrupoRepository
    {
        public ProdutoGrupo GetById(int id)
        {
            ...
            {
                var sql = "Select IdPdvGrupo as GrupoId, Descricao, Grupo_Cor as GrupoCor, IsPadrao from pdvGrupos Where IdPdvGrupo = @id ";
        ...
        public IEnumerable<ProdutoGrupo> GetAll()
        {
            using (var cn = Connection)
            {
                var sql = "Select IdPdvGrupo as GrupoId, Descricao, grupo_img as Imagem, Grupo_Cor as GrupoCor, IsPadrao from pdvGrupos Order By Sequencia";
